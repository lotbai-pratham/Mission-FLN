...
const schoolsFromCsv_1 = require("../prisma/schoolsFromCsv");
const prisma = new client_1.PrismaClient();
const udiseMap = new Map(schoolsFromCsv_1.schoolsFromCsv.map(s => [s.name, s.udise]));
// Helper to convert English strings or Marathi string to levels
function parseLiteracyLevel(val) {
    if (!val)
        return 0;
    const str = String(val).toLowerCase();
    if (str.includes('beginner') || str.includes('प्रारंभिक'))
        return 0;
    if (str.includes('letter') || str.includes('अक्षर'))
        return 1;
    if (str.includes('word') || str.includes('शब्द'))
        return 2;
    if (str.includes('paragraph') || str.includes('उतारा'))
        return 3;
    if (str.includes('story') || str.includes('गोष्ट'))
        return 4;
    return 0;
}
function parseNumeracyLevel(val) {
    ...
    if (str.includes('1-9') || str.includes('1 ते 9') || str.includes('number 1-9'))
        return 1;
    // 10-99
    if (str.includes('10-99') || str.includes('10 ते 99') || str.includes('number 10-99'))
        return 2;
    if (str.includes('subtraction') || str.includes('वजाबाकी'))
        return 3;
    if (str.includes('division') || str.includes('भागाकार'))
        return 4;
    return 0;
}
...
function processFile(filename) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            // 1. Division
            let division = yield prisma.division.findFirst({ where: { name: divName } });
            if (!division) {
                division = yield prisma.division.create({ data: { name: divName } });
            }
            // 2. Project Office
            let po = yield prisma.projectOffice.findFirst({ where: { name: poName, divisionId: division.id } });
            if (!po) {
                po = yield prisma.projectOffice.create({ data: { name: poName, divisionId: division.id } });
            }
            ...
            if (!school) {
                ...
                if (existingUdise) {
                    school = yield prisma.school.findUnique({ where: { udiseCode: existingUdise } });
                }
                if (!school) {
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            yield processFile('baseline.xlsx');
            yield processFile('midline.xlsx');
            yield processFile('endline.xlsx');
