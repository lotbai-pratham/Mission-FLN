        ...
        <div className={`space-y-6 transition-opacity animate-in slide-in-from-bottom-4 duration-500 ${isPending ? 'opacity-50' : ''}`}>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[48px] p-10 border border-slate-100 dark:border-slate-800 shadow-2xl">
            ...
            {leaderboard.length > 0 && (() => {
              // Group by class and calculate ranks with ties
              const groups = leaderboard.reduce((acc, stu) => {
                if (!acc[stu.classNum]) acc[stu.classNum] = [];
                acc[stu.classNum].push(stu);
                return acc;
              }, {} as Record<number, any[]>);

              const groupedLeaderboards = Object.entries(groups)
                .sort(([a], [b]) => Number(a) - Number(b))
                .map(([classNumStr, group]: [string, any]) => {
                  let currentRank = 1;
                  let currentScore = -1;
                  let sameScoreCount = 0;
                  const ranked = (group as any[]).map((stu: any, i: number) => {
                    if (i === 0) {
                      currentScore = stu.totalScore;
                      return { ...stu, _rank: 1 };
                    }
                    if (stu.totalScore === currentScore) {
                      sameScoreCount++;
                      return { ...stu, _rank: currentRank };
                    }
                    currentRank += sameScoreCount + 1;
                    currentScore = stu.totalScore;
                    sameScoreCount = 0;
                    return { ...stu, _rank: currentRank };
                  });
                  return { classNum: Number(classNumStr), students: ranked };
                });

              return groupedLeaderboards.map(({ classNum, students }: { classNum: number, students: any[] }) => {
                const podium = students.filter((s: any) => s._rank <= 3);
                const tableList = students.filter((s: any) => s._rank > 3);

                return (
                  <div key={classNum} className="mb-16 last:mb-0">
                    <h3 className="text-xl font-black text-slate-800 dark:text-slate-200 mb-6 flex items-center gap-2">
                      <span className="px-3 py-1 bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-400 rounded-lg text-sm">
                        {t('Class')} {classNum}
                      </span>
                      {t('Leaderboard')}
                    </h3>

                    {/* Top 3 Podium Card Grid (Handles Ties) */}
                    {podium.length > 0 && (
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        {podium.map((stu: any) => {
                          const rankIdx = Math.min(stu._rank - 1, 2); // 0=1st, 1=2nd, 2=3rd
                          ...
                          return (
                            <div key={stu.id} className="relative bg-slate-50 dark:bg-slate-850 hover:bg-white dark:hover:bg-slate-800 rounded-[32px] p-6 border border-slate-100 dark:border-slate-800 shadow-lg hover:shadow-xl transition-all hover:-translate-y-1 flex flex-col justify-between overflow-hidden">
                              ...
                              <div className={`absolute top-0 right-0 px-5 py-2.5 bg-gradient-to-r ${medalColors[rankIdx]} text-[10px] font-black rounded-bl-3xl shadow-sm uppercase tracking-wider`}>
                                {podiumTitles[rankIdx]}
                              </div>
